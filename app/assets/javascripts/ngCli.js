ngCli = angular.module('ngCli', ['duScroll', 'ngResource', 'ngAnimate', 'ui.bootstrap']);

ngCli.filter('capitalize', function() {
...
ngCli.controller('paginator', function($scope, $document, $filter, $http, $q, postRes) {
	$scope.pageSize = 5;
	...
	$scope.goTo = function(target) {
		return $document.scrollToElementAnimated(target, 120);
	};
});
...
ngCli.directive('willScroll', function ($timeout, $animate) {
    return {
        restrict: 'A',
        	...
        	function go(id) {
				...
				if (target) { 
					$scope.goTo(target).then(function(res) {
						$animate.addClass(target, 'pulse blue').then(function() {
							$timeout(function(){ 
...
ngCli.controller('accordion', function($scope, postRes) {
	$scope.data = postRes.query({action: 'recent'});
	$scope.active = undefined;
	$scope.isOpen = function(month) {
		if (month.posts && month.posts.length > 0 && !$scope.active) { 
			$scope.active = month;
			return true;
		}else {
			return false;
